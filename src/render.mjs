...
function joinForm(title, formType = "recruitment") {
  ...
  const subject = isContact ? "Kontakt fra nettsiden" : "Bli med i Asker Symfoniorkester";
  ...
  <form class="form glass" action="mailto:post@askersymfoniorkester.no" method="post" enctype="text/plain">
    <h2 class="display">${title}</h2>
    <input type="hidden" name="emne" value="${subject}">
    ...
    <label for="${formType}-topic">${topicLabel}<input id="${formType}-topic" name="${topicName}" required></label>
