...
function joinForm(title) {
  ...
  <form class="form glass">
    <h2 class="display">${title}</h2>
    <label>Navn<input></label>
    <label>Instrument / tema<input></label>
    <label>E-post<input></label>
    <label>Melding<textarea rows="7"></textarea></label>
    <button type="button" class="btn">Send melding</button>
  </form>
</div>`;
}
...
export function pages() {
  ...
  out.push([
    "/kontakt/",
    simplePage("Kontakt", "Ta kontakt med orkesteret", `${joinForm("Kontaktskjema")}<div class="card"><h3>Kontaktpunkter</h3><p class="list">Generell kontakt: post@askersymfoniorkester.no<br>Rekruttering: post@askersymfoniorkester.no<br>Facebook: <a href="${facebookUrl}">åpne side</a><br>Kart/venue-plassholder: Asker og omegn</p></div>`, "/kontakt/")
  ]);
