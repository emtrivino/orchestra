...
function footer() {
  return `<footer class="site-footer">
    <div class="container footer-grid">
      ...
      <section><h2>Besøk</h2><p>${link("/konserter/", "Konserter")}<br>${link("/sesongen/", "Sesongen")}<br>${link("/aktuelt/", "Aktuelt")}<br>${link("/repertoar/", "Repertoar")}</p></section>
      <section><h2>Delta</h2><p>${link("/bli-med/", "Bli med")}<br>${link("/stott-oss/", "Støtt oss")}<br>${link("/medlemmer/", "For medlemmer")}<br>${link("/musikere/", "Musikere")}</p></section>
      <section><h2>Kontakt</h2><p><a href="mailto:${contactEmail}">${contactEmail}</a><br><a href="${facebookUrl}" rel="noopener">Facebook</a><br>Asker, Norge</p></section>
    </div>
  </footer>`;
}
...
export function pages() {
  const pagesOut = [["/", home()]];
  pagesOut.push(["/konserter/", simplePage("Konserter", "Program", "Kommende konserter med Asker Symfoniorkester.", `${sectionIntro("Oversikt", "Kommende konserter")}<div class="event-list">${concerts.map((c) => concertCard(c, c === concerts[0])).join("")}</div>`) ]);

  for (const c of concerts) {
    pagesOut.push([`/konserter/${c.slug}/`, simplePage(c.title, c.type, c.desc, `<div class="detail-layout"><article class="panel"><p class="kicker">Program</p><h2>Musikken</h2><ul>${c.programme.map((work) => `<li>${esc(work)}</li>`).join("")}</ul><p class="kicker">Solister</p><ul>${c.soloists.map((soloist) => `<li>${esc(soloist)}</li>`).join("")}</ul></article><aside class="panel quiet"><h2>Praktisk informasjon</h2><p><strong>${fmt(c.date)}</strong><br>kl. ${c.time}<br>${c.venue}<br>Dirigent: ${c.conductor}</p>${c.ticketUrl ? `<a class="button" href="${c.ticketUrl}" rel="noopener">Billetter</a>` : link("/kontakt/", "Spør om billetter", "button")}</aside></div>`) ]);
  }
